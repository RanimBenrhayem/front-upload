...
const showJoinedTab = joinedTab ? <button  className="buttonShow" onClick={JoinedFiles}>show</button> : <button  className="buttonShow" onClick={()=>{
  ...
  return (
    <>
      ...
      <div className="descr">This option is used to make a join between 2 files already found in your simple files list table.<div>In the case where you want to make a join between 2 files that do not exist in the table, you must redo the process from the beginning.</div>
      <div>The join can't be apply when the choice of the attributs is wrong ..!</div></div>
      {showFile && (
                 ...
                 {showJoinedTab}
                  ...
                  <CSVLink
                    ...
                    onClick={(event, done) => {
                      ...
                      const blob = new File([JSON.stringify(showFile)], joinFileName, {
                        type: "text/csv",
                      });
