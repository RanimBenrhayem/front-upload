...
import {getUserSimpleFiles,deleteSignleFiles,downloadFiles,joinProcess,uploadJoinFiles,getUserJoinedFiles} from "../../services/axios"
...
function swall (){
  Swal.fire({
    title: "This option is used to make a join between 2 files already found in'your files' table.In the case where you want to make a join between 2 files that do not exist in the table, you must redo the process from the beginning"
  })
}
...
// const buttonshow = showFile ? <Button  style={{    backgroundColor: '#007FFF',color : '#ffffff',} } onClick={{setShow(false) ; handleShow("")}}>Show Joined Files</Button> :  <Button  style={{    backgroundColor: '#007FFF',color : '#ffffff',} }>Show Joined Files</Button> 
  ...
  async  function JoinedFiles (){
    ...
    const response  = await joinProcess(file1ToJoin,file2ToJoin,attribut1,attribut2)
    if (response.success ===true){
      console.log(  response.data.joinedResult)
      const headerKeys2 = Object.keys(Object.assign({}, ...response.data.joinedResult));
   ...
   const headersString = headerKeys2.reduce((previousHeader,currentHeader)=>previousHeader+","+currentHeader , "")+"\n"
    ...
    {files.map((element,index)=> {
      ...
      // </thead>
         return (
           ...
           <SingleFilePreview id={element._id}  handleShow={handleShow} isDeleted={isDeleted} setIsDeleted={setIsDeleted} />
  ...
  <th>Delete </th>
    ...
    {joinedFiles.map((element,index)=> {
        ...
        /> */}
           ...
           <JoinedFilePreview id={element._id}  handleShow={handleShow} isDeleted={isDeletedJoinFiles} setIsDeleted={setIsDeletedJoinFiles} />
...
<Button  className="Button" onClick={handleClick} style={{
        ...
        variant="contained" >Want to join files..?</Button>
...
<div className="box">
{headers1.length>0 && (
  <select value={attribut1} onChange={(e)=>setAttribut1(e.target.value)}> 
...
{headers2.length>0 && (
  ...
  <select value={attribut2} onChange={(e)=>setAttribut2(e.target.value)}> 
  <option value={''}>_please select an attribut_</option>
    {headers2.map((element,index)=> {
     ...
     <div>
     <button className='buttonShow'onClick={JoinedFiles}>show</button>



    
<CSVLink data={showFile} filename={joinFileName}>
	<button className="download" >Download<AiOutlineCloudDownload/></button>
</CSVLink>
<CSVLink
  ...
  onClick={(event, done) => {
    ...
    const blob = new File(showFile,joinFileName , {type:"text/csv"})
